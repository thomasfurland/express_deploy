defmodule <%= @app_module %>.MixProject do
  ...
  defp aliases do
    [
      setup: ["deps.get"<%= if @ecto do %>, "ecto.setup"<% end %>, "assets.setup", "assets.build"]<%= if @ecto do %>,
      ...
      "assets.deploy": <%= inspect Enum.map(@asset_builders, &"#{&1} default --minify") ++ ["phx.digest"] %>
    ]
  end
end
